...
注意 `PMingLiU` 那一列：只比 `MingLiU` 多一個 `P`。`P` = proportional，
半形字的寬度本來就不保證固定——這才是它不能用的原因。上表那個 2.13 是本機
量到的值，實際數字會隨 Windows 解析到哪一個字面而變（CI 的 runner 上量到的
是 2.00），所以測試驗的是「它不是等寬字型」這個穩定性質，而不是某個比例數字。
同理還有 `MS PGothic`：它的比例剛好是 2.00，但一樣是比例字型。名字差一個
字母、量出來又未必看得出來，所以這條規則是寫成測試擋著的，不是靠 code review。
...
之類的標準名稱一個都拿不到），而 `QStyle` 的內建圖示沒有書籤／摺疊／巨集／
欄選取這些概念。三個平台長得一樣，比各自符合當地習慣更重要。
...
- **工具列**：每顆按鈕都有圖示檔、沒有沒人用的圖示、圖示是單色而且尺寸一致、
...
載入翻譯有一個不明顯的副作用：**Qt 猜 menuRole 用的關鍵字也會跟著翻譯**。
Qt 預設會拿選單文字去比對 "Quit"／"Exit"／"About" 之類的字，決定要不要把
那一項搬進應用程式選單；載入繁中翻譯後，它眼中的 "Quit" 與 "Exit" 都變成
「離開」——於是「離開欄模式」被判定成結束，搶走了應用程式選單的結束位置，
按「結束 Stephany Editor」實際執行的是離開欄模式，程式關不掉。

所以每個動作都明確指定 `menuRole`，一個都不留給 Qt 去猜（只有「關於」與
「結束」有真正的角色，其餘一律 `NoRole`）。測試會掃過整個視窗確認這件事，
並且會自己跟著介面文字走：日後新增的選單項只要撞到翻譯後的關鍵字而沒設
`NoRole`，就會紅燈。
